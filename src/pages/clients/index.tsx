...
const Clients = () => {
        ...
        <Space>
          <ViewBtn
            ...
            onClick={async () => {
              ...
              navigate(`details/${record?.CustNo}`, { state: record });
            }}
          />
          <EditBtn
            loading={loading.includes("delete")}
            onClick={async () => {
              actions.getDetails(record);
              navigate(`update/${record?.CustNo}`, { state: record });
            }}
          />

          <DeleteBtn
            onConfirm={async () => {
              await actions.deleteClient(record?.CustNo);
            }}
            loading={loading.includes("delete")}
          />
        </Space>
  ...
  const { actions, list, loading, query } = useContext(ClientContext);
  ...
  return (
    <>
      <PageHeader
        title={"العملاء"}
        subTitle={"جميع العملاء المسجلون"}
        extra={[
          <Button
